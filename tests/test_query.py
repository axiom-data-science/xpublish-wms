...
def test_get_map_colormap(opt):
    ...
    assert request.styles.palettename != "colormap" and request.styles.palettename != "default"


def test_get_map_arrows():
    getmap_query = WMSGetMapQuery.model_validate({
        "service": "WMS",
        "version": "1.3.0",
        "request": "GetMap",
        "layers": "layer1",
        "styles": "raster/arrows",
        "crs": "EPSG:3857",
        "tile": "1,1,1",
        "width": 100,
        "height": 100,
    ...
    request = GetMap(cache=None, array_render_threshold_bytes=1e9)
    request.ensure_query_types(xr.Dataset({'layer1': [1,2,3]}), getmap_query, {})
    assert request.styles.type == "arrows"
