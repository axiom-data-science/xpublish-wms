...
from xpublish_wms.wms.get_map.vector_styles import get_cell_center_indices, visualize_vectors
...
class GetMap:
    ...
    def render(
        ...
        try:
            ...
            x_span = abs(self.bbox[2] - self.bbox[0])
            y_span = abs(self.bbox[3] - self.bbox[1])
            x_buffer = x_span * max(0.15, (self.margin_px / self.width) if self.width else 0)
            y_buffer = y_span * max(0.15, (self.margin_px / self.height) if self.height else 0)
            bbox = [
