...
from xpublish_wms.query import GET_MAP_STYLE_METHODS, WMSGetMapQuery, WMSGetMetadataQuery
...
def get_layer_details(ds: xr.Dataset, layers: List[str]) -> dict:
    ...
    if ds.gridded.has_elevation(da1):
        elevation = np.atleast_1d(ds.gridded.elevations(da1).values).round(5).tolist()
    ...
    return {
        ...
        "standard_name": ",".join(da.cf.attrs.get("standard_name", da.name) for da in das),
